...
async function rebuildFlashcards(v, get, SYS) {
  ...
  if (outFile) {
    ...
    for (let i = 0; i < old.length; i++) {
      ...
      // 卡片身份不含展示细节(音频嵌入、<br> 之后的例句),否则改一次卡面格式
      // 就会让所有卡的复习进度归零
      const back = m[2].replace(/!\[\[[^\]]*\]\]/g, "").split("<br>")[0].trim();
  ...
  const out = ["---", "tags:", "  - english-study", "  - flashcards", "---", "",
    ...
    "> 复习需要 **Spaced Repetition** 插件(可选插件,没装就先装:设置 → 第三方插件 → 浏览 → 搜它;用默认设置,尤其别改卡组标签 `#flashcards`)。",
    "> 装好后:命令面板搜 `flashcards` → 「Spaced Repetition: Review flashcards from all notes」。",
    "> 插件只能自评「记得/不记得」;真正的产出式练习在每天 ChatGPT 热身的「表达重测」里。", "",
    cards.length || noCue.length ? `共 ${cards.length} 张卡片。`
      : "\n## 现在还是空的,这是正常的\n\n闪卡来自表达库。练几天、攒下**带中文释义**的条目后,这里会自动长出卡片。\n条目格式:`- expression *(YYYY-MM-DD,中文意思)*` —— 没有释义的做不成卡片(卡片正面就是那个中文提示)。\n"];
