...
if (looksLike) {
  ...
  if (!/^# /m.test(body)) {
    body = body.replace(/^[^\n]*English Study[^\n]*\n?/, "");
    const secs = ["Speaking Time","Main Topics","Strengths","Key Corrections","Error Log","New Words & Phrases","Model Sentence","Tomorrow's Suggestion & Topic","Tomorrow’s Suggestion & Topic","Overall Evaluation"];
